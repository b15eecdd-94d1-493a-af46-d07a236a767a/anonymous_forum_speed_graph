...
class PostingInThreadSpeedDownloader:
    ...
    def get_posting_speed(self):
        ...
        if 'posts_count' not in json_data:
            print("Ключ 'posts_count'не найден в JSON.")
            return []
        ...
        try:
            posting_speed_data = {
                'posts_count': json_data['posts_count'],  # Количество постов за час
...
if __name__ == "__main__":
    ...
    downloader = PostingInThreadSpeedDownloader(url, filename)
    rating_last_index = 0
    while(True):
        try:
            ...
            if posting_speed_data:
                ...
                if args.show_statistic == 'Y':
                    ...
                    for entry in rating:
                        ...
                        print(f"Постов: {entry['posts_count']}, Дата и время: {local_time}")
                    rating_last_index += i
