...
class PostingSpeedAnalyzer:
    def __init__(self, data, title = 'Скорость постинга', filename = 'board_speed.png', datetime_format = '%Y-%m-%d %H:%M:%S', timezone = 'UTC', interval_minutes = 0, interval_hours = 0, interval_days = 0, width = False, height = False):
        """
    ...
    def plot_posting_speed(self):
        ...
        for entry in self.data:
            ...
            if prev_entry_datetime == False:
                ...
                posts.append(entry['board_speed'])
            else:
        ...
        plt.ylabel('Количество постов за час')
        plt.grid(True)
    ...
    try:
        if os.path.exists(rating_file) and os.path.getsize(rating_file) > 0:
            with open(rating_file, 'r') as f:
                ...
                analyzer = PostingSpeedAnalyzer(data, 'График скорости постинга в /' + Path(rating_file).stem + '/', Path(rating_file).stem, args.datetime_format, args.timezone, args.interval_minutes, args.interval_hours, args.interval_days, args.width, args.height)
